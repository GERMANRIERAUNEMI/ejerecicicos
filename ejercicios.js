class Proyecto {
    ...
    perfectos() {
        let d = document
        let serie = [5, 6, 28, 1, 3]
        let $resp = d.getElementById("Salida")
        let sum = 0; let c = 0

        for (c = 0; c < serie.length; c++) {
            let num = serie[c];

            if (this.divisores(num)) {
                sum = sum + num;
            }
        }

        $resp.textContent = "La suma es: " + sum;
    }
    ...
    Primos() {
        let d = document
        let num1 = d.getElementById("Entrada").value.split(",");
        console.log(num1)
        let $resp = d.getElementById("Salida");
        let arregloprimos = [];

        for (let c = 0; c < num1.length; c++) {
            let num = parseInt(num1[c]);

            if (this.isPrimo(num) == 1) {
                arregloprimos.push(num);
            }
        }

        $resp.textContent = "Los números primos son: " + arregloprimos.join(",");
    }
    ...
    ejercicio4() {
        ...
        serieNumeros = serieNumeros.split(",")
        let arreglonumeros = []
